...
- The subscription-gated AI/billing model: Thock users bring their own key. _(2026-09-11: partially walked back on Thock's own terms, the optional Thock Plus hosted agent of §4.3, while BYO stays the free default.)_
...
6. **Distribution & model:** open-source core? paid Routines? one-time vs. subscription (for the app; intelligence optional, never required)? _(Partially answered 2026-09-11: a **Thock Plus** subscription gates the features that cost money to serve: hosted agent first, sync storage later; BYO intelligence stays free. Spec `specs/v25-thock-plus-hosted-agent.md`.)_
...
- [x] **Invisible git, checkpoint service:** background snapshots to the hidden `.thock/history` git-dir (`crates/thock/src/history.rs`). _(shipped)_
...
- [x] **Checkpoint triggers:** on open, idle, heartbeat, before every agent session (both the BYO and the hosted panel call it), and on close. _(shipped)_
...
- [ ] **Skill contract & write sandbox:** scopes are _declared_ in the manifest but not yet enforced. Per-write previews were dropped with V25 decision 16, so enforcement means the OS sandbox on the hosted path (V25 Stage 2). _(planned)_
...
- [x] **A template you can edit without breaking sync** — the planner heading stopped being an exact string match. Headings resolve on a normalized key (links reduced to their label, everything that isn't a letter or digit folded to a space), so `## 📅 Day planner`, `## Day planner:`, `## **Day planner**` and `## Day-planner` all find the section, in any language; extra *words* still don't, because the reconciler writes where it resolves. `[day_planner] heading` now also takes a list — new name first, the old one kept as an alias — which is how a rename or a Set Language translation keeps yesterday's notes parsing. The same matcher backs the Backlog's configured headings. And the silent failure is gone: a hold carries a structured reason, so the Day Planner's status row names the heading it wanted and offers **Add heading** (writes it into today's note at the note's own level) and **Use another…** (points the config at a heading the note already has, through `toml_edit`, so the user's comments survive); a level-6 planner heading, which can never hold a Calendar child, now says so instead of doing nothing. Spec `specs/v26-planner-heading-resolution.md`. _(shipped)_
...
- [x] **Subscription core, billing-agnostic:** a small backend (`thock/services/plus`, Go on Postgres with embedded migrations) with plans as hot-changeable rows (allowance in normalized units, model-tier mapping, per-plan limits, edited through the admin API and live on the next request), its own users/entitlements tables and usage ledger, per-user budget-capped OpenRouter keys minted and disabled through the provisioning API (a fake gateway stands in when no management key is set), and dev entitlements via admin-minted invite codes. The allowance loop runs end to end without Polar: usage syncs from the gateway, 80% warns, zero disables the key, and admin endpoints reset, top up, or revoke any user. Spec `specs/v25-thock-plus-hosted-agent.md`. _(shipped, Stage 1)_
- [x] **Hosted Thock Agent chat panel:** a Thock-owned right-dock chat panel (`crates/thock/src/chat_panel.rs`, `thock::ToggleChatFocus`) over the inherited ACP crates, hosting **Pi** through the `pi-acp` adapter, both installed with the managed Node runtime into a Thock-owned folder on first use. "Connect Thock Agent" takes an invite code, stores the credential in the keychain, and injects the per-user gateway key, the tier's model, and a note-taking system prompt through a private Pi config directory; no keys or model names ever reach the UI. Every permission the harness asks for is granted (no approval prompts); the checkpoint service snapshots the vault before each session; tool activity streams in plain language with diff previews; the footer shows the allowance bar, warns at 80%, and refuses at zero; a revoked entitlement falls back to the free terminal path. Run actions route to the chat when the hosted agent is the chosen connection (`[agent] mode`), and the BYO terminal panel is untouched beside it. Fully keyboard-driven (arrows, `j`/`k`, enter, escape). _(shipped, Stage 1; sandbox pending Stage 2)_
- [x] **Chat that reads like a conversation:** the V26 refinement of the chat panel. Everything the agent does between two messages collapses into one quiet activity line — composed by the renderer from the run's tool kinds, live in present tense while the turn runs — that expands in place to per-call rows with vault-relative paths and diff previews. No tool names, absolute paths, or raw errors ever reach the collapsed transcript; per-call failures are carried by the agent's prose, with one plain-language turn-level line (and retry) as the floor. Reasoning leaves no residue, the agent's questions render inline and are answerable by keyboard, and the user's message sits in a visible bubble. Expansion is keyboard-first (`left`/`right`, vim `h`/`l`). Spec `specs/v26-agent-chat-refinement.md`. _(shipped)_
- [x] **An agent that knows whose vault it is:** the hosted session prompt (V27). `SYSTEM.md` is the agent's identity — who it is to someone who does not program, how it speaks, the rules that hold even in a vault whose `AGENTS.md` was edited away, and an honest account of what it cannot do (it names a note rather than claiming to open one). A generated `APPEND_SYSTEM.md` carries what only this launch knows: today's date and ISO week, today's and this week's note paths, the backlog file with its *configured* headings, the day-plan heading, whether a `profile.md` exists, the language the Set Language ritual recorded, and every installed Routine with its rituals. Pi loads both before the vault's own `AGENTS.md`, so the user's file keeps the last word. Spec `specs/v27-agent-session-prompt.md`. _(shipped)_
- [x] **An agent that remembers:** a `memory/` folder at the vault root, plain Markdown, in three tiers: `memory/index.md` always in context under a configured line cap (`[memory] index_lines`, appended to the V27 context block as *What you already know*), detail pages (`people/`, `projects/`, `preferences.md`, `patterns.md`) read on demand with a date and source on every line, and the daily notes themselves found by search. Written by the core **Reflect** ritual (`skills/thock/reflect.md`, Fast tier) that ends Wrap Today, Wrap Yesterday and Week Review and has its own row in the Timeline Routine, never by per-message extraction; in-session the agent only appends one-liners to `memory/inbox.md`, and the chat panel suggests Reflect after a few sessions with unfiled notes. Facts are kept silently and forgotten only with a tick; imported text never becomes a belief; `profile.md` gains **What Thock should not keep**; `AGENTS.md` carries the same rule for BYO agents. **Rebuild Memory** (`skills/thock/rebuild-memory.md`, `thock::RebuildMemory`) builds `memory/` from an existing vault a fortnight at a time and resumes where it stopped. Spec `specs/v28-agent-memory.md`. _(shipped; Fast-tier eval pending)_
- [x] **Updates that reach existing vaults:** shipped Routine docs and skills and the core files (`AGENTS.md`, the core rituals) are replaced in place on every reconcile when their on-disk hash still matches what the app last shipped (`files.lock` per Routine, new `.thock/core-files.lock` for the core), so most upgrades land with no agent at all. A file the person edited is never overwritten: the newer shipped version is staged under `.thock/pending/` once per shipped version (`.thock/updates.lock`), the Routines rail shows an "N rituals to update" row, and the **Update Rituals** core ritual (`skills/thock/update-rituals.md`, `thock::UpdateRituals`) merges each one with their edits, carries `## Language` over untouched, and asks before writing. Spec `specs/v29-update-rituals.md`. _(shipped)_
