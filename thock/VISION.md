...
- [x] **Subscription core, billing-agnostic:** a small backend (`thock/services/plus`, Go, standard library only) with plans as hot-changeable config (`plans.json`: allowance in normalized units, model-tier mapping, per-plan limits, re-read on edit), its own users/entitlements store and usage ledger, per-user budget-capped OpenRouter keys minted and disabled through the provisioning API (a fake gateway stands in when no management key is set), and dev entitlements via admin-minted invite codes. The allowance loop runs end to end without Polar: usage syncs from the gateway, 80% warns, zero disables the key, and admin endpoints reset, top up, or revoke any user. Spec `specs/v25-thock-plus-hosted-agent.md`. _(shipped, Stage 1)_
