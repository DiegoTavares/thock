...
**Owner:** Diego · **Date:** 2026-09-09, revised 2026-09-11
**Companion docs:** `../VISION.md` (§4.3 "Bring your own brain", amended by this spec; §9 trust model; §12 roadmap), `v5-agent-and-onboarding.md` (the BYO rails this builds beside, whose "no ACP client work" non-goal this supersedes), research brief: https://claude.ai/code/artifact/b6ac65e0-8012-4a51-aac2-f920a4598b7d
...
Delivery is **two stages, Polar last**. Stage 1 is the whole agent experience (ACP + Pi +
...
## 2. Locked decisions (2026-09-09, revised 2026-09-11)
...
| 8 | Meter unit | **Normalized units (cost-in-cents based)**, not raw tokens, so the allowance survives model switches. |
...
| 13 | VISION | Amended with this spec (2026-09-11), VISION artifact republished at its existing URL: BYO stays first-class; §10's "never the intelligence" now reads "intelligence optional, never required". |
...
| 15 | Delivery order | **Two stages, Polar last.** Stage 1 = ACP + Pi + OpenRouter + chat panel on dev entitlements: a full iteration loop with zero billing setup. Stage 2 = Polar (checkout, license keys, credits) + safety hardening (sandbox, ceilings, revocation). |
...
5. **Trust without gates**: no per-change approval prompts anywhere (decision 16). Instead, every
   hosted session requests a pre-session checkpoint from the invisible-history service (the V2
   history service is shipped and the BYO panel already calls it), the panel shows what the
...
- **No-gates trust model leans on the checkpoint service**: the service ships (`history.rs`, with
  a pre-session trigger both panels call), but the restore UI does not yet, so an errant session
  is recoverable only by someone who knows the hidden history exists. Prioritize the restore
  surface; append-don't-rewrite remains convention, not enforcement.
