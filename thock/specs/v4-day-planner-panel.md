# Thock V4 — Day Planner Context Panel
...
Rationale: the sample `templates/daily.md` already ships a `## Day planner` section (`v1-daily-panel.md` §5.3), so notes created by Thock get scoped parsing for free, while hand-made notes still work.
...
- **New crate module, new right-dock panel.** Add `crates/thock/src/day_planner_panel.rs` (the `Panel`) and a pure `crates/thock/src/day_plan.rs` (parsing + geometry, no GPUI) so the parse/layout logic is unit-testable without a window. Register the panel next to `TimelinePanel` in `thock.rs`'s `init`/`add_panel_when_ready` wiring. Keep the diff isolated to the `thock` crate plus the one registration site (VISION §7.1 small-fork mandate).
