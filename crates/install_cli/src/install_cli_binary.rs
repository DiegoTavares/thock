...
actions!(
    cli,
    [
        /// Installs the Thock CLI tool to the system PATH.
        InstallCliBinary,
    ]
);
...
async fn install_script(cx: &AsyncApp) -> Result<Option<PathBuf>> {
    ...
    let link_path = Path::new("/usr/local/bin/thock");
...
pub fn install_cli_binary(window: &mut Window, cx: &mut Context<Workspace>) {
    const LINUX_PROMPT_DETAIL: &str = "If you installed Thock from our official release add ~/.local/bin to your PATH.\n\nIf you installed Thock from a different source like your package manager, then you may need to create an alias/symlink manually.";
                        ...
                        |cx| {
                            cx.new(|cx| {
                                MessageNotification::new(
                                    "You can add `thock` to your PATH manually.",
                                    cx,
                                )
                                .with_title("Couldn't install the Thock CLI")
        ...
        workspace.update_in(cx, |workspace, _, cx| {
            ...
            workspace.show_toast(
                Toast::new(
                    ...
                    format!(
                        "Installed `thock` to {}. You can launch {} from your terminal.",
    ...
    .detach_and_prompt_err("Cannot install the Thock CLI", window, cx, |_, _, _| None);
}
