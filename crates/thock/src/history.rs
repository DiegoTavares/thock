//! Invisible checkpoint history for Thock vaults (spec: v2-invisible-git).
//!
//! A background service that commits snapshots of the whole vault into a
//! hidden git repository whose git-dir is `<vault>/.thock/history`.
...
const AUTHOR_NAME: &str = "Thock";
const AUTHOR_EMAIL: &str = "history@thock.local";
...
const BASE_EXCLUDES: &str = "/.thock/history/\n/.git/\n";
...
impl HistoryService {
    ...
    fn finish_checkpoint(
        ...
        match result {
            ...
            Err(error) => {
                log::debug!(
                    "Thock history: checkpoint of {} failed (will retry on the next trigger): {error:#}",
                    state.root.display()
                );
...
fn is_history_repo_path(unix_path: &str) -> bool {
    let history_prefix = ".thock/history";
...
mod tests {
    ...
    fn history_repo_paths_are_filtered() {
        assert!(is_history_repo_path(".thock/history"));
        assert!(is_history_repo_path(".thock/history/objects/ab/cd"));
        assert!(!is_history_repo_path(".thock/config.toml"));
        assert!(!is_history_repo_path(".thock/history-notes.md"));
