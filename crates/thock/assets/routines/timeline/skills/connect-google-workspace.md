...
Link a Google account to this vault with one sign-in that powers three rituals: today's accepted meetings appear inside today's daily note (as ordinary Markdown checklist lines under the Day planner section), any email the user labels **`thock/backlog`** in Gmail is archived into the vault and becomes a task in the Backlog's Someday column, and anything flung at Google Tasks from a phone — or labeled **`thock/inbox`** in Gmail — lands as a note in the vault's `inbox/` folder for the Triage Inbox ritual. Thock itself does the syncing; this skill's job is to get the connection made and explain what the user will see.

**Reads:** Google Calendar, Gmail, and Google Tasks — all read-only.
**Writes:** `.thock/google.toml` (the account), `.thock/calendar.toml` / `.thock/gmail.toml` / `.thock/inbox.toml` (per-feature preferences), the `Calendar` subsection of today's daily note, `backlog.md` (Someday, append), `archives/emails/` (captured emails), `inbox/` (captured items).
...
- The gesture lives in Gmail: label an email `thock/backlog`, and within a few minutes its text is archived as a plain Markdown note under `archives/emails/` and the thread appears once as an unchecked task at the end of the Backlog's **Someday** section — carrying an Obsidian-style `[[wikilink]]` to the archive and its own invisible `<!--gmail:…-->` identity comment. Label it `thock/inbox` instead when it matters but isn't yet a task — it becomes an inbox note for triage. An email carrying both labels takes the backlog lane, once.
- Labels route, folders mean: `.thock/gmail.toml` maps each Gmail label to the folder its emails land in, and adding a `[[sync]]` entry there wires a brand-new label to a brand-new folder with no other setup.
...
- `.thock/gmail.toml` — `poll_seconds` plus ordered `[[sync]]` entries, each a `label` → `path` pair. The defaults are `thock/backlog` → `archives/emails` (notes landing there also get a Someday line in `backlog.md`) and `thock/inbox` → `inbox`; a thread carrying several mapped labels is captured once, by the first entry.
- `.thock/inbox.toml` — `dir = "inbox"` (the landing zone; keep it matching the inbox `[[sync]]` path in `gmail.toml`), `poll_seconds`, `[tasks] list` (defaults to the account's default list) / `enabled`.
