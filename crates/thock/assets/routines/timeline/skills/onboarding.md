...
> This file is written for the AI agent Thock launches when the Daily &
...
2. Propose a mapping to the vault's configured daily filename format (default
   ...
   encode their convention in `.thock/config.toml` (`[daily]` /
...
1. Create the file `.thock/state/onboarded/timeline` (create the parent
   directories if needed). Write a short version of the report as its body —
   one paragraph is plenty. This is how Thock knows setup finished.
